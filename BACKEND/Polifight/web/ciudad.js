...
// Model for Ciudad entity
    models.Ciudad = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Polifight/webresources/entitities.ciudad/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idCiudad');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Polifight/webresources/entitities.ciudad/';
            }
    ...
    // Collection class for Ciudad entities
    models.CiudadCollection = Backbone.Collection.extend({
        model: models.Ciudad,
        url: "http://localhost:8080/Polifight/webresources/entitities.ciudad/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CiudadCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-ciudad-list-itemis template identifier for item
                        templateName: '#tpl-ciudad-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.ciudad = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.ciudad,
                    // tpl-ciudad-details is template identifier for chosen model element
                    templateName: '#tpl-ciudad-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Ciudad();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-ciudad-details is a template identifier for chosen model element
                templateName: '#tpl-ciudad-details',
