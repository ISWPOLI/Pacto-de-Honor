...
// Model for Poder entity
    models.Poder = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Restful.PH/webresources/com.poder.restful.ph.poder/",
        defaults: {
            tipoPoder: "",
            nombrePoder: "",
            formaPoder: "",
        ...
        toViewJson: function() {
            ...
            result.displayName = this.get('tipoPoder');
            return result;
        },
        isNew: function() {
        ...
        sync: function(method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/Restful.PH/webresources/com.poder.restful.ph.poder/';
            }
    ...
    // Collection class for Poder entities
    models.PoderCollection = Backbone.Collection.extend({
        model: models.Poder,
        url: "http://localhost:8080/Restful.PH/webresources/com.poder.restful.ph.poder/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function() {
            this.collection = new models.PoderCollection();
            var self = this;
            this.collection.fetch({
                success: function() {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-poder-list-itemis template identifier for item
                        templateName: '#tpl-poder-list-item'
                    });
        ...
        details: function(id) {
            if (this.collection) {
                this.poder = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.poder,
                    // tpl-poder-details is template identifier for chosen model element
                    templateName: '#tpl-poder-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Poder();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-poder-details is a template identifier for chosen model element
                templateName: '#tpl-poder-details',
        ...
        getData: function() {
            return {
                ...
                nombrePoder: $('#nombrePoder').val(),
                formaPoder: $('#formaPoder').val(),
                potenciaPoder: $('#potenciaPoder').val(),
                descripcionPoder: $('#descripcionPoder').val()
            };
        }
    });
    new AppRouter();
