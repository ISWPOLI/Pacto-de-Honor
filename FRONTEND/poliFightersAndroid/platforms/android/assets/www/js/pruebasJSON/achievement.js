/*Este archivo se crea para declarar los logros del jugador como se va a hecer en la base de datos
*Con esto nos aseguramos que vaya a funcionar cuando de consuman los servicios 
*/
var SetUplogros ={
	"nombre": [	"Mi KuPlagio", "Mi LifePlagio ", "Mi SkillPlagio",
				"The originalPlagio ", "Mi HitBackground  \n“Es poseedor de la experiencia, “Campeón de habilidades”", 
				"Mater \n Maestro del Pacto",
				"The Richest  \n“Podrás presumir tu fortuna”", 
				"The Unlocker \n“Eres el encargado para recuperar a todos los héroes”", 
				"The A – List \n“Aquí mostrarás tus habilidades al llegar al top”", 
				"5 in a row \n“Cambio de un mundo a otro con un desempeño perfecto”"
				],
	"descripcion":[  "Para adquirir este codiciado objetivo debes acumular 100 horas de juego\n¿Cuestión de tiempo no? \nAl final sólo queda un dilema, ¿termino una carrera, o el juego? ",
                     "Su nombre lo dice todo. \nPara obtenerlo debes encabezar las tablas de posición.\nObtendrás 100 monedas por la permanencia durante 2 días \nen las primeras cinco posiciones de la tabla.\n No hay vía fácil, no hay camino rápido.\nSimplemente debes ser el mejor en los combates. ¿Te atreverías a intentar conseguirlo?",
                     "Para obtenerlo debes tener desbloqueado un 70% de los logros.",
                     "Para adquirir este codiciado logro \ndebes acumular 3 batallas victoriosas \n¿Cuestión de estrategia no? Pues para obtener el logro \ndebes realizar dicho proceso antes de la batalla final de cada nivel.",
                     "Su nombre lo muestra todo.\nPara obtenerlo debes Derrotar al jefe del nivel mundo 1 en menos de 1 minuto.\nObtendrás por ello puntos de experiencia que servirán para subir de nivel.\nNo hay camino fácil.Simplemente debes ser uno de los mejores en el campo de batalla.\n¿Te atreverías a arrebatarlo?",
                     "Para obtener este logro debes tener 100000 pto de Experiencia como minimmo",
                     "Obten este cuando tu fortuna llegue a 100000 monedas. ¡Un verdadero Magnate!",
                     "Adquiere a todos los personajes.",
                     "Para obtener las recompensas de este logro debes llegar al top 1 del Rankin.",
                     "Para obtener este logro de perfección es necesario \nque pases todo un mundo sin perder ni una sola vez. \n¿Crees ser capaz de cumplir el reto?",
                     ],
    "jugador_tiene_logros":["The Unlocker \n“Eres el encargado para recuperar a todos los héroes”","Mi SkillPlagio","Mater \n Maestro del Pacto"],
    "monedas":100,
    "ranking":7,
    "tiempo":500,
	"exp":1000
}
	